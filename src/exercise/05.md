...
## 📝 What I've learnt
- If your context `value` changes frequently, a quick way to improve performance is to [memoize it](https://github.com/HelpMe-Pls/react-performance/blob/master/src/final/05.js) with `React.useMemo`.
- A better alternative is to [separate](https://github.com/HelpMe-Pls/react-performance/blob/master/src/final/05.extra-1.js) the state and the mechanism for updating that state into two separate contexts (at line `44-45`).
