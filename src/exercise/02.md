...
## 📝 What I've learnt
- Compare the Performance between [having](https://github.com/HelpMe-Pls/react-performance/blob/master/src/exercise/02.js) `useMemo` and [not](https://github.com/HelpMe-Pls/react-performance/blob/master/src/final/TS/02.tsx): open the Chrome DevTools “Performance” tab, click the settings gear and set CPU from “no throttling” to “6x slowdown.” Then select any item from the dropdown. Next, click the “Record” button in the devtools to start a recording. Then wait for 5s and click on “force rerender” and then click the “Record” circle again to stop the recording. You’ll notice that the `useMemo` eliminates the `getItems` function call after we "force rerender", therefore, brings better performance.
