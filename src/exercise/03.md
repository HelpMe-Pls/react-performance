...
- Try to compare the "Profiler" between [having](https://github.com/HelpMe-Pls/react-performance/blob/master/src/exercise/03.js) a memoized component and [not]() having it: you’ll notice the `<ListItem/>` no longer re-rendered when you click on the "force rerender" button, saving React the work of having to call the `ListItem`  function and compare the previous react elements with the new ones. (`<Menu>` still get re-rendered because it takes the `item` prop which depends on the `<App/>`)
